...
describe('users - promotionCodes - use', () => {
  beforeEach(() => {
    // @ts-ignore
  ...
  test('fails - prmotionCode is not a string with more than one characters', async () => {
    global.fetch = jest.fn(async () => ({
      ok: true,
      json: async () => ({ test: true }),
    })) as jest.Mock;
  ...
  test('succeeds - can be used with bearer token on client', async () => {
    ...
    const response = await userdocks.users.promotionCodes.use(
      '408edd4e-ef97-49f6-8a02-7f325e897096',
      '408edd4e-ef97-49f6-8a02-7f325e897096',
    );
