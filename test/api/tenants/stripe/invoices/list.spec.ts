...
describe('tenants - stripe.invoices - list', () => {
  beforeEach(() => {
    // @ts-ignore
  ...
  test('fails - tenantId is not a uuid', async () => {
    ...
    const response = async () => userdocks.tenants.stripe.invoices.list('a');
  ...
  test('succeeds - can be used with bearer token on client', async () => {
    ...
    const response = await userdocks.tenants.stripe.invoices.list(
      '408edd4e-ef97-49f6-8a02-7f325e897096',
    );
